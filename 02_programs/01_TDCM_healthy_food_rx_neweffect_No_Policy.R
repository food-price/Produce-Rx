...
#Secondary Authors: Brianna Lauren, Lu Wang
#Contact: brianna.lauren@tufts.edu; Lu.wang@tufts.edu
#Main Purpose: 
...
#Updates (01/01/2022): Improved model efficiency by incorporating parallel computing, with summarizing partly done before combining simulations/
#Updates (03/01/2022): Further incoporated the effect of produce RX on BMI and HbA1c 
#Updates (06/01/2022): Updated baseline death probability, by estimating deaths conditional on desease status 
#Updates (07/01/2022): Calibrated model to match the predicted prevalence with observed prevalence by race/ethncity. 
...
NHANES<- fread("01_Input/NHANES/NHANES_1318_Imp100_dm_fi.csv", stringsAsFactors = TRUE, data.table = FALSE)
...
# 1.3 Select the target starting population to model (Age 40-79) 
...
# 1.5-1 Policy-effect size (based on 9 intervention studies)
 # policy_effect_fruit_sim <- calc_nsims_rbeta(n.sim, mu = 0.31, se = 0.18)
 # policy_effect_veg_sim <- calc_nsims_rbeta(n.sim, mu = 0.31, se = 0.18)

#update policy effect with kurts' results added
...
policy_effect_BMI_sim <- rnorm(n.sim, -0.36,  0.10)


# Policy costs
c_policy = 30.86 * 12 # weighted average of dollar amount in 10 studies included in the meta-analysis (annual, 2021 dollar)
...
source("02_Programs/3_sim_function_new.R")
...
saveRDS(sim_out, file = paste("03_Output/sim_out",intervention,  "SEED", seed, n.sim, n.cycle, Sys.Date(), ".rda", sep = "_"))
